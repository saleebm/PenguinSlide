...
The fix is a budget, not fewer haptics. PenguinSlide gives haptic feedback three jobs and explicitly refuses to do anything else with it.
...
Both generators are kept as long-lived instance properties on `IcicleSystem`, and `prepare()` is called in `init`:
