...
- **First access is suddenly heavier.** `static var Penguin: PenguinTuning = .loadFromUserDefaults()` runs the JSON decode on the first reference. Fine for our scale (one decode at app start). The old `static let` was zero-cost at access.
...
This is a tiny refactor with two careful choices, not a "tuning system":
...
The win is *unblocking* tunability, not tunability itself. We didn't ship sliders yet, but the day we want to, every wiring decision is already made. That's what the refactor bought.
