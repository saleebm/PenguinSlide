...
One detail worth highlighting: the **8 fps** in the manifest is the same number that lives in `PenguinTuning.animationFps`. The asset and the runtime agreed on the playback rate before I wrote a single `SKAction.animate(with:timePerFrame:)`. Not a coincidence — telling SpriteCook the target frame rate up front is what made the animations feel right when SpriteKit played them back.
